  ...
  double& E_out, double mu, uint64_t* seed)
{
  ...
  if (CoherentElasticAE* coherentElasticAE =
    ...
    //  std::cout << "Used " << typeid(*coherentElasticAE).name() << "
    ...
    // std::ofstream
    // outFile("/root/OpenMC/projects/openmc/paper/runs/disk2/mCH2_deg60_E0.025_rho1/pdf.txt");
    ...
    if (creat_pdf_file) {
      std::ofstream outFile("/home/itay/Documents/openmc/projects/tests/"
                            "all_iso/thick_sphere/Graphite/pdf.txt");
    ...
    std::cout << "Used " << typeid(*incoherentElasticAE).name()
              << " implementation." << std::endl;
    ...
    std::cout << "Used " << typeid(*incoherentElasticAEDiscrete).name()
              << " implementation." << std::endl;
    ...
    // std::cout << "Used " << typeid(*incoherentInelasticAEDiscrete).name() <<
    // " implementation (water for example)" << std::endl;
    ...
    bool creat_pdf_file = false;
    if (creat_pdf_file) {
      std::ofstream outFile("/root/OpenMC/projects/openmc/paper/runs/thermal/"
                            "runs/mwater_deg60_E0.025_rho1/pdf.txt");
      ...
      for (double E = h2o_Ein_min; E <= h2o_Ein_max; E *= log_step) {
        for (int l = 0; l <= 63; ++l) {
          for (double mu = -1.0; mu <= 1.0; mu += 0.01) {
            // Calculate PDF using get_pdf method
            double pdf1 =
              (*incoherentInelasticAEDiscrete).get_pdf(E, E_out, mu, seed, l);
            ...
            ++current_iteration;
            double progress =
              static_cast<double>(current_iteration) / total_iterations;
            int bar_width = 50;
